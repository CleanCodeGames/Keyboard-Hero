...
public:
	...
	Level_24(void)
	{
		...
		CreateBonusBar(23,25,29,34);

		count_key = 0;
		count_key_capture = 0;
		m_player->setKeyCount(count_key_max);

		ConstructText(text_welcome, v2f(0,-35*scr_1), 10, sf::Color::Green, L"\"Chance\"", font_modeka);
	...
	virtual void Update()
	{
		...
		case STATE::play:
			...
			if(count_key < count_key_max)
			{
				...
				if(timer_key_spawn > 1650/modifier_speed)
				{
					if(key_type < 2)
					{
						uint type = rand()%4;
			...
			Teleport("T_1S", "T_1E", v2f(0.008,0));
			Teleport("T_2S", "T_1E", v2f(0.008,0));
			break;
		}
	}
