...
public:
	...
	Level_40(void)
	{
		ConstructText(text_welcome, v2f(0,-35*scr_1), 10, sf::Color::Green, L"\"Rockfall 2\"", font_modeka);
		vec_BG.push_back(make_shared<BackGround_L8>(BackGround_L8()));
		music = &sound->BossTheme;
		CreateBonusBar(36,42,42,52);
		percent_need = 98;
		count_key_max = 240;
		count_key = 0; 
		count_key_capture = 0;
	...
	virtual void Update()
	{
			...
			case STATE::play:
				...
				if(count_key < count_key_max)
				{
					...
					if(timer_key_spawn > 900/modifier_speed)
					{
						...
						if(count_key != 10) KeyGenWhiteBlack(12, 12);
						else isBoss = true;
					}
				}
				break;
		...
		else
		{
			if(state == play && vec_Key.empty() && vec_Boss.empty()) 
				vec_Boss.push_back(make_shared<Boss_2>(Boss_2(125, 750)));
		}
	}
