...
public:
	...
	Level_28(void)
	{
		...
		CreateBonusBar(36,42,42,50);

		count_key = 0;
		count_key_capture = 0;
		m_player->setKeyCount(count_key_max);

		ConstructText(text_welcome, v2f(0,-35*scr_1), 10, sf::Color::Green, L"\"Lotus 2\"", font_modeka);
				...
				if(count_key < count_key_max)
				{
					...
					if(timer_key_spawn > 1000/modifier_speed)
					{
						if(key_type < 8)
						{
							...
							v2f siz(7,7), spd = v2f(cosf(_a), sinf(_a))*0.013f;
